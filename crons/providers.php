...
$there_change=0;
          ...
          $extras = json_decode($service["api_detail"],true);
            ...
            if( $apiService["max"] != $extras["max"] ):
              ...
              if( $service["api_servicetype"] == 1 && $there[$service["service_id"]] ):
                ...
                $update = $conn->prepare("UPDATE services SET api_detail=:detail, api_servicetype=:type, service_type=:typex WHERE service_id=:service ");
                $update->execute(array("service"=>$service["service_id"],"detail"=>$detail,"type"=>2,"typex"=>2 ));
                $insert = $conn->prepare("INSERT INTO serviceapi_alert SET service_id=:service, serviceapi_alert=:alert, servicealert_date=:date, servicealert_extra=:extra ");
  ...
  foreach ($there as $service => $type):
    ...
    if( $type == 0 && $serviceDetail["api_servicetype"] == 2 ):
      ...
      $update = $conn->prepare("UPDATE services SET api_servicetype=:type, service_type=:typex WHERE service_id=:service ");
      $update->execute(array("service"=>$service,"type"=>1,"typex"=>1 ));
      $insert = $conn->prepare("INSERT INTO serviceapi_alert SET service_id=:service, serviceapi_alert=:alert, servicealert_date=:date, servicealert_extra=:extra ");
