...
function serviceSpeed($speed,$price){
    ...
    switch ($speed) {
        case '1':
            return '<span style="color: #f24236;font-weight: 500;">'.priceFormat($price).'<span style="font-size:10px;"  class="fa fa-arrow-down"> </span></span>';
            break;
        case '2':
            return '<span style="color: #fe6d86;font-weight: 500;">'.priceFormat($price).'<span style="font-size:10px;"  class="fa fa-arrow-down"> </span></span>';
            break;
        case '3':
            return '<span style="color: #5696c9;font-weight: 500;">'.priceFormat($price).' <span style="font-size:10px;" class="fa fa-compress"> </span></span>';
            break;
        case '4':
            return '<span style="color: #0dd887;font-weight: 500;">'.priceFormat($price).' <span style="font-size:10px;"  class="fa fa-arrow-up"> </span></span>';
            break;
    }
}
...
function dayPayments($day,$ay,$year,$extra=null){
    ...
    $row    = $conn->query("SELECT SUM(payment_amount) FROM payments WHERE payment_delivery='2' && payment_status='3' && payment_create_date<='$last' && payment_amount>0 && payment_create_date>='$first' $where  ")->fetch(PDO::FETCH_ASSOC);
