...
if ((route(1) == "login" || route(1) == "register") && $_SESSION["userlogin"]) {
...
if (route(1) == "neworder" || route(1) == "orders" || route(1) == "tickets" || route(1) == "addfunds" || route(1) == "account" || route(1) == "dripfeeds" || route(1) == "reference" || route(1) == "subscriptions") {
  ...
  if ($captcha && $settings["recaptcha"] == 2 && $captcha_control->success == false) {
    ...
    $insert = $conn->prepare("INSERT INTO clients SET client_id=:client_id, name=:name, username=:username, email=:email, password=:pass, lang=:lang, telephone=:phone, register_date=:date, apikey=:key ");
    $lastid = $conn->prepare("SELECT client_id FROM clients ORDER BY client_id DESC LIMIT 1");
    $lastid->execute();
    $lastid = $lastid->fetch()["client_id"] + 1;
    $insert = $insert->execute(array("lang" => $selectedLang, "client_id" => $lastid, "name" => $name, "username" => $username, "email" => $email, "pass" => md5(sha1(md5($pass))), "phone" => $phone, "date" => date("Y.m.d H:i:s"), 'key' => $apikey));
    if ($insert) :
      $client_id = $lastid;
    endif;
